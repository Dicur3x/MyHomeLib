...
Начиная с pre5.06 дополнительно проверяются размеры и смещения полей
`TLocalStatusChangedMessage` относительно `TMessage`, затем пять уведомлений
после загрузки в настоящее скрытое окно VCL и отдельное уведомление о снятии
локального статуса. Проверка ABI выполняется до разыменования указателя:
прежний Win64-формат даёт понятную ошибку теста вместо Access violation.

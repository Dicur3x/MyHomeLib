...
//
// Ленивое расширение схемы: GUID версии не меняем, поэтому старые сборки
// открывают мигрированную коллекцию, а старая коллекция добавляет колонки
// при первом открытии. Транзакция не нужна: каждый ALTER атомарен,
// а проверка существования колонки делает процедуру идемпотентной.
//
...
function TBookCollection_SQLite.InsertAuthorIfMissing(const Author: TAuthorData;
  Cache: TImportCache): Integer;
var
  Key: string;
begin
  if not Assigned(Cache) then
    Exit(InsertAuthorIfMissing(Author));
...
function TBookCollection_SQLite.FindOrCreateSeries(const Title: string;
  Cache: TImportCache): Integer;
var
  NormalizedTitle: string;
begin
  if not Assigned(Cache) then
    Exit(FindOrCreateSeries(Title));
  if NO_SERIES_TITLE = Title then
    Exit(NO_SERIES_ID);

  NormalizedTitle := UpperCase(Trim(Title));
...
function TBookCollection_SQLite.InsertBook(BookRecord: TBookRecord;
  const CheckFileName, FullCheck: Boolean): Integer;
begin
  Result := InternalInsertBook(BookRecord, CheckFileName, FullCheck, nil);
end;

function TBookCollection_SQLite.InsertBook(BookRecord: TBookRecord;
  const CheckFileName, FullCheck: Boolean; Cache: TImportCache): Integer;
begin
  Result := InternalInsertBook(BookRecord, CheckFileName, FullCheck, Cache);
end;

function TBookCollection_SQLite.InternalInsertBook(BookRecord: TBookRecord;
  const CheckFileName, FullCheck: Boolean; Cache: TImportCache): Integer;
const
  SQL_INSERT =
    'INSERT INTO Books (' +
...
const
  ...
  SQL_DELETE_ORPHAN_AUTHORS =
    'DELETE FROM Authors ' +
    'WHERE AuthorID IN (' +
    '  SELECT al.AuthorID FROM Author_List al WHERE al.BookID = ?' +
    ') AND NOT EXISTS (' +
    '  SELECT 1 FROM Author_List other ' +
    '  WHERE other.AuthorID = Authors.AuthorID AND other.BookID <> ?' +
    ')';
var
  ID: Integer;
  OwnTransaction: Boolean;
begin
  ...
  begin
    ...
    if OwnTransaction then
      FDatabase.Start;
    try
      // Count every relation of each selected author. Restricting the count to
      // this book first would delete authors still used by other books.
      FDatabase.ExecSQL(SQL_DELETE_ORPHAN_AUTHORS, [ID, ID]);
    ...
    // The system database is separate, so update it only after the collection
...
begin
  ...
  insertedIds := TDictionary<Integer, Boolean>.Create;
  try
    query := FDatabase.NewQuery(SQL_INSERT);
    try
      for Author in Authors do
      begin
        if not insertedIds.ContainsKey(Author.AuthorID) then
        begin
            query.SetParam(0, Author.AuthorID);
            query.SetParam(1, BookID);
            query.ExecSQL;
            insertedIds.Add(Author.AuthorID, True);
        end;
      end;
    finally
      query.Free;
...
begin
  ...
  insertedCodes := TDictionary<string, Boolean>.Create;
  try
    query := FDatabase.NewQuery(SQL_INSERT);
    try
      for Genre in Genres do
      begin
        if not insertedCodes.ContainsKey(Genre.GenreCode) then
        begin
          query.SetParam(0, BookID);
          query.SetParam(1, Genre.GenreCode);
          query.ExecSQL;
          insertedCodes.Add(Genre.GenreCode, True);
        end;
      end;
    finally
      query.Free;

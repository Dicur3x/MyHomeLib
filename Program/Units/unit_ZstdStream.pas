(* *****************************************************************************
  ...
  * Created             22.08.2026
  * Description         Поток чтения zstd через libzstd.dll (динамическая
  *                     загрузка: без DLL недоступен только импорт metabib)
  *                     libzstd.dll: официальные сборки facebook/zstd v1.5.7
...
resourcestring
  rstrZstdDllMissing = 'Не найдена библиотека %s. Импорт каталогов metabib недоступен.';
  rstrZstdError = 'Ошибка zstd: %s';
  rstrZstdTruncated = 'Файл zstd обрезан или повреждён.';
...
begin
  ...
  FLastRet := 1; // внутри кадра, пока не доказано обратное
end;
...
//
// Заполняет FOutBuf следующей порцией. False — данные закончились.
//
...
begin
  ...
  repeat
    ...
    begin
      ...
      begin
        if FLastRet = 0 then
          Exit(False); // кадр завершён, источник исчерпан — штатный конец
...
begin
  // Поддерживаем только запрос позиции (TStream.Position); перемотки нет.

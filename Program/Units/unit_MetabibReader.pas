(* *****************************************************************************
  ...
  * Created             22.08.2026
  * Description         Чтение каталогов metabib (JSON Lines: заголовок
  *                     metabib.dataset/1 + записи metabib.dataset_record/1).
  *                     Контейнеры: .jsonl, .jsonl.zst, .jsonl.gz, .zip
  *
  ****************************************************************************** *)
...
resourcestring
  rstrNotMetabibDataset = 'Файл не является каталогом metabib (ожидался metabib.dataset/1): %s';
  rstrNoJsonlInZip = 'В ZIP-архиве нет файла *.jsonl: %s';

const
  DATASET_SCHEMA = 'metabib.dataset/1';
  RECORD_SCHEMA = 'metabib.dataset_record/1';

  // Аннотация из самой книги предпочтительнее текста из базы библиотеки.
  ANNOTATION_SOURCES: array [0 .. 2] of string = ('fb2', 'fbd', 'db');

  // Для авторов, переводчиков, названия и серий база первична:
  // в FB2 встречаются повреждённые имена с символами U+FFFD.
  CATALOG_SOURCES: array [0 .. 2] of string = ('db', 'fb2', 'fbd');

// ---------------------------------------------------------------------------
// Вспомогательный разбор "claim array": каждое поле claims — массив
// {observation, value, raw}; value бывает скаляром или массивом.
// ---------------------------------------------------------------------------
...
// Keep every series from the first source with usable names. Mixing sources
...
var
  claim: TJSONObject;
  v, el: TJSONValue;
  list: TList<TMetabibSequence>;
  source: string;

  procedure AddSequence(Item: TJSONValue);
  var
    nv: TJSONValue;
...
begin
  source := '';
  list := TList<TMetabibSequence>.Create;
  try
    for claim in ClaimsInOrder(Group, 'sequences', Order) do
    begin
      if (list.Count > 0) and
        not SameText(source, StrValue(claim, 'observation')) then
        Continue;
      v := claim.Values['value'];
      if v is TJSONArray then
      begin
        ...
        AddSequence(v);
      if list.Count > 0 then
        source := StrValue(claim, 'observation');
    end;
    Result := list.ToArray;
  finally
    list.Free;
...
begin
  ...
  if Ext = '.zip' then
  begin
    // ZIP может быть и INPX-подобным архивом: каталог metabib определяем
    // по элементу *.jsonl (INPX содержит structure.info и *.inp)
    try
      ...
      // не ZIP или архив повреждён — пусть дальше обрабатывает штатный импорт
    end;
  end;
...
//
// Читает одну строку, двигаясь только вперёд: TStreamReader здесь не подходит,
// потому что на границе буфера откатывает поток назад, а zstd-поток не умеет Seek.
//
...
var
  ...
  begin
    ...
    if FFirstLine and (Length(FPending) >= 3) and (FPending[0] = $EF) and
      ...
      FPending := Copy(FPending, 3, Length(FPending) - 3); // BOM, если каталог создан с ним
    try
      Line := TEncoding.UTF8.GetString(FPending);
    except
      on EEncodingError do
        Line := '{'; // намеренно битые байты — направляем по пути mrBadLine
    end;
    FFirstLine := False;
  end;
...
begin
  ...
  begin
    ...
    if i < FInLen then
    begin
      FInPos := i + 1; // пропускаем LF
      EmitPending;
      Exit(True);
    end;

    FInPos := FInLen; // строка продолжается в следующем буфере
  end;
...
begin
  if not ReadRawLine(Line) then
    raise EMetabibError.CreateFmt(rstrNotMetabibDataset, ['(пустой файл)']);
  Inc(FLineNo);

...
22 августа 2026 года этот порядок был повторно проверен на RAD Studio 13 Trial:

- `Release`, `Windows 64-bit`;
- Virtual Treeview 8.3 из GetIt;
- отдельная сборка DLL значков, основной программы и MCP-сервера;
- основной проект: `Success`, 0 ошибок, 8 предупреждений;
...
- после добавления официальной x64 `sqlite3.dll` чистый запуск показывает
  русскоязычный мастер создания коллекции; заставка больше не перекрывает его;

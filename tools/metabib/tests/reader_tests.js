...
const checks = [
  ...
  ['the database title wins even when the FB2 claim comes first', () => {
    const b = run(record(bib({ title: [
      { observation: 'fb2', value: 'Archive title' },
      { observation: 'db', value: 'Catalog title' },
    ] })));
    return b.title === 'Catalog title';
  }],

  ['a damaged database title falls back to the FB2 title', () => {
    const b = run(record(bib({ title: [
      { observation: 'db', value: '\uFFFD ()' },
      { observation: 'fb2', value: 'Archive title' },
    ] })));
    return b.title === 'Archive title';
  }],

  ['author spellings from different sources are not combined', () => {
    const b = run(record(bib({ authors: [
      { observation: 'fb2', value: [{ last_name: 'Smith', first_name: 'J.' }] },
      { observation: 'db', value: [
        { last_name: ' Smith ', first_name: 'Jane' },
        { last_name: 'Smith', first_name: 'Jane' },
        { last_name: 'Jones', first_name: 'Alex' },
      ] },
    ] })));
    return b.authors.length === 2 && b.authors[0].first === 'Jane'
      && b.authors[1].last === 'Jones';
  }],

  ['damaged people fall back to usable names and decode nickname entities', () => {
    const claims = [
      { observation: 'db', value: [{ last_name: '\uFFFD', first_name: '()' }] },
      { observation: 'fb2', value: [{ nick_name: ' &quot;Pen Name&quot; ' }] },
    ];
    const b = run(record(bib({ authors: claims, translators: claims })));
    return b.authors.length === 1 && b.authors[0].nick === '"Pen Name"'
      && b.translators.length === 1 && b.translators[0].nick === '"Pen Name"';
  }],

  ['all database series survive source ranking with wrapped numbers', () => {
    const b = run(record(bib({ sequences: [
      { observation: 'fb2', value: [
        { name: 'Old series spelling', number: 91 },
      ] },
      { observation: 'db', value: [
        { name: 'Primary series', number: { value: 2 } },
        { name: 'Secondary series', number: { value: '7' } },
      ] },
    ] })));
    return b.series === 'Primary series' && b.series_no === 2
      && JSON.stringify(b.sequences) === JSON.stringify([
        { name: 'Primary series', number: 2 },
        { name: 'Secondary series', number: 7 },
      ]);
  }],

  ['empty database series fall back to every usable FB2 series', () => {
    const b = run(record(bib({ sequences: [
      { observation: 'db', value: [{ name: ' () \uFFFD ', number: 99 }] },
      { observation: 'fb2', value: [
        { name: '', number: 1 },
        { name: ' First ', number: 3 },
        { name: 'Second', number: 4 },
      ] },
      { observation: 'fbd', value: [{ name: 'Ignored', number: 5 }] },
    ] })));
    return b.series === 'First' && b.series_no === 3
      && b.sequences.length === 2 && b.sequences[1].name === 'Second';
  }],

  ['separate claims from the chosen source preserve distinct series', () => {
    const b = run(record(bib({ sequences: [
      { observation: 'db', value: { name: 'Primary', number: 1 } },
      { observation: 'fb2', value: { name: 'Ignored', number: 9 } },
      { observation: 'db', value: { name: 'Secondary', number: { value: 2 } } },
      { observation: 'db', value: { name: 'Primary', number: 1 } },
    ] })));
    return JSON.stringify(b.sequences) === JSON.stringify([
      { name: 'Primary', number: 1 },
      { name: 'Secondary', number: 2 },
    ]);
  }],

  ['legacy unranked series claims preserve all their relationships', () => {
    const b = run(record(bib({ sequences: [
      { value: { name: 'Primary', number: 1 } },
      { value: { name: 'Secondary', number: 2 } },
    ] })));
    return b.series === 'Primary' && b.series_no === 1
      && b.sequences.length === 2 && b.sequences[1].name === 'Secondary';
  }],

  ['unusable series leave both legacy and multiple-series fields empty', () => {
    const b = run(record(bib({ sequences: [
      { observation: 'db', value: [null, {}, { name: '\uFFFD ()' }] },
    ] })));
    return b.series === '' && b.series_no === 0 && b.sequences.length === 0;
  }],

  ['publisher series are separate from author cycles', () => {
    const b = run(record({
      bibliographic: { title: [{ value: 'Book' }], sequences: [
        { observation: 'fb2', value: [{ name: 'Author cycle', number: 3 }] },
      ] },
      publication: { sequences: [
        { observation: 'fb2', value: [
          { name: 'Print & Classics', number: 6001 },
          { name: 'Pocket library', number: { value: '12' } },
        ] },
      ] },
    }));
    return b.series === 'Author cycle' && b.series_no === 3
      && b.sequences.length === 1
      && JSON.stringify(b.publisher_sequences) === JSON.stringify([
        { name: 'Print & Classics', number: 6001 },
        { name: 'Pocket library', number: 12 },
      ]);
  }],

  ['author cycles never become publisher series', () => {
    const b = run(record(bib({ sequences: [
      { value: [{ name: 'Author cycle', number: 3 }] },
    ] })));
    return b.series === 'Author cycle' && b.publisher_sequences.length === 0;
  }],

  ['publisher claims rank sources without mixing alternative spellings', () => {
    const b = run(record({ publication: { sequences: [
      { observation: 'db', value: [{ name: ' () \uFFFD ', number: 1 }] },
      { observation: 'fbd', value: [{ name: 'Old spelling', number: 7 }] },
      { observation: 'fb2', value: [{ name: 'Print series', number: 2 }] },
      { observation: 'fb2', value: { name: 'Secondary', number: 5 } },
    ] } }));
    return JSON.stringify(b.publisher_sequences) === JSON.stringify([
      { name: 'Print series', number: 2 },
      { name: 'Secondary', number: 5 },
    ]);
  }],
  ...
  ['malformed publisher numbers do not wrap or reject a book', () => {
    const b = run(record({ publication: { sequences: [{ value: [
      ...
      { name: 'Huge', number: '999999999999999999999999999999' },

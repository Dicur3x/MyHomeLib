...
// Test harness for unit_MetabibReader. Not shipped.
// Delphi Trial users build MetabibTest.dproj in the IDE.
//
...
uses
  ...
  System.IOUtils,
  System.JSON,
  unit_MetabibReader in '..\..\..\Program\Units\unit_MetabibReader.pas',
  unit_ZstdStream in '..\..\..\Program\Units\unit_ZstdStream.pas',
...
function SequencesJSON(const Sequences: TArray<TMetabibSequence>): TJSONArray;
var
  sequence: TMetabibSequence;
begin
  Result := TJSONArray.Create;
  for sequence in Sequences do
    Result.AddElement(TJSONObject.Create
      .AddPair('name', sequence.Name)
      .AddPair('number', TJSONNumber.Create(sequence.Number)));
end;
    ...
    try
      ...
      try
        ...
        repeat
          ...
          Obj.AddPair('sequences', SequencesJSON(Book.Sequences));
          Obj.AddPair('publisher_sequences', SequencesJSON(Book.PublisherSequences));
